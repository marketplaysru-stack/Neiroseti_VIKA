...
Генерация постов по схеме, обрезка текста для caption.
Только ручные темы (topics.txt или запасной список).
RSS полностью отключён.
"""
...
# ---------- TELEGRAM (с обрезкой caption) ----------
def send_message(chat_id, text, photo_bytes=None):
    if photo_bytes:
        # Обрезаем текст до 1000 символов (для caption)
        caption = text[:1000]
        if len(text) > 1000:
            caption += "... (продолжение в следующем сообщении)"
